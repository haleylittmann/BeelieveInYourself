...
void setPixel(int x, int y, u16 color) {
    ...
    // x is num of col : y is num of row
    // OFFSET(rows, col, rowlength)
}
...
void drawImageDMA(int x, int y, int width, int height, const u16 *image) { //iterate through each row
    for (int r = 0; r < height; r++) {
        DMA[3].src = image + OFFSET(r, 0, width); // one row of the image
        DMA[3].dst = videoBuffer + OFFSET(y + r, x, 240); // one row of the screen
...
void fillScreenDMA(volatile u16 color) {
    DMA[3].src = &color; // holds the address, NOT the pixel value
    DMA[3].dst = videoBuffer;
